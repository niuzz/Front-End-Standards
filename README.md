...
此为达内科技集团技术支持和教学督导部达内求职秀前端开发团队遵循和约定的代码书写规范，意在提高代码的规范性和可维护性。  
...
#### 2.1.1 项目结构  
    --- 
     |---- images/                图片资源//详见图片资源结构    
     |---- js/                    JS //具体见JS细化结构                 
     |---- style/                 CSS //具体见CSS细化结构
     |---- static/                静态资源 //具体见静态资源细化结构
     |---- public/                公用加载资源 //具体见公用加载资源细化结构
#### 2.1.2 class和id的使用
1. 页面样式应全部样式附着于 `class="xxx"` **注：** _此时文件中不包含任何一个 id="xxx"_
2. 为上一步书写 CSS style  

3. 开始书写`js`交互文件，使用 `ID` 和 `Class` 定位被操作句柄
4. 向 html或jsp中需要的地方添加 `id="xxx"` 及 `data-xxx="xxx"`  


#### 2.1.3 命名规范  
* 文件及文件夹: 全部英文小写字母+数字或连接符"`-` , `_` "，不可出现其他字符
如：`../style/style.css, jquery.1.x.x.js` 
* 文件：调用 `js/libs` 文件需包含版本号，压缩文件需包含`min`关键词，其他插件则可不包含
